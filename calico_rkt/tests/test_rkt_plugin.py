...
class RktPluginTest(unittest.TestCase):
    ...
    @patch('calico_rkt.env', autospec=True)
    @patch('sys.stdin',  autospec=True)
    @patch('calico_rkt.create', autospec=True)
    def test_main(self, m_create, m_stdin, m_env):
    ...
    def test_create_calico_endpoint(self, m_con_add, m_host, m_env):
        ...
        m_client = Mock()
        m_client.get_endpoint.return_value = 'Good'
        ...
        m_client.get_endpoint.assert_called_once_with(hostname='HOST', 
                                                    orchestrator_ID='rkt', 
                                                    workload_id=id_)
        m_con_add.assert_called_once_with(hostname='HOST', 
                                        orchestrator_ID='rkt', 
                                        container_id=id_, 
                                        netns_path=path_, 
    ...
    #             patch.object(self.plugin, "_create_profile",
    #                 autospec=True) as m_create_profile:
    #         # Set up mock objects
    ...
    #         m_create_profile.assert_called_once_with("endpt_id")
    #
    ...
    #
    #         # Call method under test
    #         self.plugin._create_profile(m_endpoint)
    #
    #         # Assert
